"""
Sweet spot emergence run: L3, visible resources, neutral prompt, arm 3x/0.5 decay.
invest_self 1%, DMs enabled. 20 rounds, 5 agents, network rewiring.
Run with: python -u sanity_emergence_sweetspot.py
"""
...
game_params = {
    ...
    'arm_cost_pct': 10,
    'arm_multiplier': 3.0,        # sweet spot: arm+attack barely +EV vs equal
    'arm_other_cost_pct': 10,
    'arm_decay': 0.5,             # bonus fades in ~3 rounds — vulnerability windows
    'attack_take_pct': 40,
    'conflict_cost_pct': 5,
...
d.print_run_header(
    f"SWEET SPOT RUN: {NUM_AGENTS} agents, {NUM_ROUNDS} rounds\n"
    ...
    f"invest_self: {game_params['invest_self_pct']}% | comm: {game_params['comm_scope']}\n"
...
for round_num in range(1, NUM_ROUNDS + 1):
    ...
    for aid in agent_ids:
        ...
        if traces:
            thinking = traces[-1].get('thinking', '')
            d.print_reasoning(aid, thinking)
    ...
    if comm != 'none':
        ...
        for aid in agent_ids:
            ...
            if msg and msg.get('message'):
                ...
                if comm == 'broadcast' or msg_to == 'all':
                    for nbr in neighbors:
                        next_messages[nbr].append({
                            'from': aid, 'message': msg['message'],
                            'channel': 'broadcast',
                        })
                    sent.append(msg)
        ...
        d.print_messages([
            {'from': m['from'], 'to': m.get('message_to', '?'),
             'message': m['message'], 'channel': 'dm' if m.get('message_to') != 'all' else 'broadcast'}
            for m in sent
        ])
...
log_data = {
    "config": {
        ...
        "reasoning_level": "level3", "comm_scope": game_params['comm_scope'],
